...
import com.alex.mysticalagriculture.cucumber.item.tool.BaseFishingRodItem;
...
public class EssenceFishingRodItem extends BaseFishingRodItem implements Tinkerable {
    private static final EnumSet<AugmentType> TYPES = EnumSet.of(AugmentType.TOOL, AugmentType.FISHING_ROD);
    private final int tinkerableTier;
    private final int slots;

    public EssenceFishingRodItem(ToolMaterial toolMaterial, int tinkerableTier, int slots, Function<Settings, Settings> settings) {
        super(settings.compose(p -> p.maxDamage(toolMaterial.getDurability())));
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        var augments = AugmentUtils.getAugments(stack);
        var success = super.postHit(stack, target, attacker);
